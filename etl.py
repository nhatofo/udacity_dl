...
def create_spark_session():
    """
       The function Initiate a spark session on AWS
       args:
            None
       Return value:
                Spark session
...
def process_log_data(spark, input_data, output_data):
    """
        The funtion process log data using spark on AWS .
        Input: 
              song_data in  .json format, 
        output:
              Processed data in parquet format loaded back to S3
        args:
              Spark session, input_data, output_data
...
def main():
    """
        Extract songs and events(logs) data from S3,
        Transform it into dimensional tables format, 
        and Load it back to S3 in Parquet format making
